...
контейнер остановился, потому что сигнал Ctrl-C (SIGINT) завершил основной процесс контейнера — в данном случае это nginx. Если основной процесс контейнера завершает свою работу, весь контейнер останавливается
...
так как мы изменили порт контейнера с 80 на 81, локальный порт хоста, который был связан c портом контейнера 80 больше не может поключиться
...
docker обнаружил, что для текущего проекта существует контейнер (task-portainer-1), который больше не указан в новом файле манифеста, и он советует очистить этот контейнер указав --remove-orphans, после всего гасим наш контейнер командой down
